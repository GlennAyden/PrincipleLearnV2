-- Align tracked Supabase schema with the research-grade discussion
-- completion and assessment model already used by the admin APIs.
...
CREATE INDEX IF NOT EXISTS idx_discussion_assessments_prompt_message_id
  ON public.discussion_assessments (prompt_message_id);

CREATE INDEX IF NOT EXISTS idx_discussion_assessments_subtopic_id
  ON public.discussion_assessments (subtopic_id);
...
COMMENT ON COLUMN public.discussion_sessions.completed_at IS
  'Time when the guided discussion requirement was completed.';

COMMENT ON COLUMN public.discussion_sessions.completion_reason IS
  'Reason the discussion was allowed to complete, such as all goals met or remediation exhausted.';

COMMENT ON COLUMN public.discussion_sessions.completion_summary IS
  'Research summary of goal coverage, weak goals, and completion metadata.';

COMMENT ON TABLE public.discussion_assessments IS
  'Research read-model for each student discussion answer assessed against each learning goal.';
